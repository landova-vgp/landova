import React from 'react';
import './ProjectContent.css'; 

const ProjectContent = () => {
  return (
    <div className="detail-content">
      <h3>Project Location: Guduvancheri – Nellikuppam</h3>
      <p><strong>Total Plots:</strong> 117 Residential Plots</p>

      <p><strong>Highlights:</strong><br />
        Gated community with compound wall<br />
        Black tar roads for easy access<br />
        Clear title, ready for registration<br />
      </p>

      <p><strong>Rate per Sq.Ft:</strong> ₹2,575<br />
      <strong>Advance Booking:</strong> ₹1,00,000 only<br />
      <strong>Loan Available:</strong> LIC Housing Finance & YES Bank</p>

      <p><strong>Special Launch Offer:</strong><br />
        Starting from just ₹2,200 per Sq.Ft.* for select units. Limited-time pricing—ideal for early investors looking to maximize appreciation.</p>

      <hr/>

      <h3>VGP LANDOVA – Invest in Keelur, the Fast-Growing Zone Between OMR & GST!</h3>

      <p>Looking to invest in a plot with high appreciation value?</p>

      <p><strong>Strategic Location:</strong><br />
        OMR – GST – Keelur<br />
        Tiruporur – Chengalpattu – Keelur<br />
        Guduvancheri – Nellikuppam – Keelur<br />
      </p>

      <p><strong>Keelur</strong> is emerging as one of the most promising real estate zones in Chennai. With rapid development in infrastructure, schools, colleges, IT hubs, and highway connectivity, this region is set to boom in the next 3 to 5 years!</p>

      <p>Government-approved projects, expanding residential communities, and upcoming industries make <strong>VGP LANDOVA plots</strong> the perfect investment choice.</p>

      <p>Backed by the 70-year legacy of the VGP Group, LANDOVA offers:<br />
        DTCP & RERA Approved Plots<br />
        Sizes from 636 Sq.Ft. Onwards<br />
        80% Bank Loan Available<br />
        Easy EMI Options<br />
        Clear Title & Immediate Registration<br />
      </p>

      <p><strong>Own with Just 25% Down Payment!</strong><br />
      Thanks to our Easy Installment Plan, owning land is now as easy as paying monthly rent. Pay just 25% upfront and finance the rest over a tenure of up to 15 years with interest rates from 10.5% p.a. A 636 Sq.Ft. plot (approx. ₹14,00,000) can be yours at an EMI of ₹11,250/month—perfect for salaried professionals and first-time buyers.</p>

      <br/>

      <hr/>

      <h3>VGP LANDOVA – Premium DTCP & RERA Approved Plots for Sale!</h3>
      <p><strong>Keelur – Near OMR & GST</strong><br />A fast-growing location backed by 70 years of VGP Group’s trusted legacy</p>

      <p><strong>Layout Highlights:</strong><br />
        117 Plots – Ranging from 636 to 2830 Sq.Ft.<br />
        Wide Black Top Roads<br />
        80-Feet Water Facility<br />
        Fully Gated Community<br />
        Compound Wall for Entire Layout<br />
        Only ₹2,575/- per Sq.Ft.<br />
      </p>

      <p>Additional features include complete drainage infrastructure, LED street lighting, clearly numbered plots, and groundwater access at just 80 feet depth—making these plots fully ready for construction from day one.</p>

      <p><strong>Registration & Document Charges Extra</strong> as per Government Guideline Value & Stamp Duty.<br />
      80% Bank Loan Available<br />
      Immediate Registration & Clear Titles</p>

      <p><strong>Tailored for Every Buyer:</strong> Whether you’re an NRI looking for long-term security, a young family planning your dream home, or an investor seeking strong rental returns, LANDOVA has something for everyone.</p>

      <p><strong>Brand Legacy:</strong> Brought to you by the trusted VGP Group—known for landmarks like VGP Golden Beach and Universal Kingdom—LANDOVA is your gateway to a secure, high-value land investment. <em>Your Land, Your Legacy.</em></p>

      <hr/>

      <h3>Thiruporur – Fast-Growing Real Estate Hotspot</h3>
      <p>Located ~48 km south of Chennai along the Old Mahabalipuram Road (OMR), Thiruporur is evolving from a quiet panchayat town into a promising investment destination.</p>

      <p><strong>What’s driving the growth?</strong></p>
      <ul>
        <li><strong>Strategic connectivity: </strong> Sandwiched between Kelambakkam and Alathur and just off OMR, it provides easy access to major IT hubs like Siruseri and Sholinganallur, making it ideal for tech professionals.</li>
        <li><strong>Infrastructure momentum: </strong>The 7.5 km Thiruporur bypass (six-lane) is now largely operational, easing congestion and improving movement along OMR and access to the coastal ECR.</li>
        <li><strong>Upcoming amenities: </strong> The proposed Chennai peripheral road and nearby attractions like the 62‑acre Wonderla theme park (slated to open in 2025) are set to boost local demand.</li>
        <li><strong>Metro on the horizon: </strong> Metro Phase II extension will eventually connect OMR suburbs like Navalur and Siruseri (in Thiruporur taluk), which is expected to positively impact property rates.</li>
      </ul>

      <p><strong>Real Estate Scene:</strong></p>
      <ul>
        <li><strong>Plots & apartments: </strong>A mix of ready-to-move apartments and plotted developments are emerging. Price trends: from ~₹2,400–3,000/sq ft a few years ago to ₹3,100–4,000/sq ft now ().</li>
        <li><strong>Developer focus: </strong>Builders like DivyaSree, Casagrand, Vijay Raja, Adityaram are actively launching projects spanning villas, apartments, and plots—from ₹15 L to ₹1 Cr+.</li>
        <li><strong>Market sentiment: </strong>Investors see Thiruporur as a top-10 land investment area in Chennai, citing OMR Metro, rise in residential demand, and future infrastructure ().</li>
      </ul>

      <p><strong>Emerging Challenges:</strong></p>
      <p>Long-term residents note growing pains: patchy sewage systems, incomplete drainage, and traffic during temple events. Public transport is still evolving.</p>

      <p><strong>Summary</strong></p>
      <p>Thiruporur is transitioning fast—from farmland to a suburban extension supporting Chennai’s booming tech-driven sprawl. Its appeal lies in urban connectivity, improving roads, Metro prospects, and organized new projects. While infrastructure still plays catch-up, the growth trajectory suggests property values are likely to keep climbing—making it a compelling spot for both end-users and investors.</p>

      <hr/>

      <h3>Directions and distances from VGP LANDOVA site at Nellikuppam village to nearby locations</h3>
      <p><strong>1. Guduvanchery ➝ Nadhivaram ➝ Nellikuppam</strong><br />
        <strong>Distance:</strong> 13.2 km<br />
        <strong>Travel Time:</strong> 23 minutes<br />
        <strong>Route:</strong> Guduvanchery ➝ Nadhivaram ➝ Kandigai ➝ Vandalur–Kelambakkam Road ➝ Nellikuppam
      </p>
      
      <p><strong>2. Nellikuppam ➝ Kottamedu</strong><br />
        <strong>Distance:</strong> 4.1 km<br />
        <strong>Travel Time:</strong> 9 minutes<br />
        <strong>Route:</strong> Nellikuppam ➝ Local internal village road ➝ Kottamedu
      </p>
      <p><strong>3. Nellikuppam ➝ Thiruporur</strong><br />
        <strong>Distance:</strong> 10.4 km<br />
        <strong>Travel Time:</strong> 19 minutes<br />
        <strong>Route:</strong> Nellikuppam ➝ Pudupakkam ➝ Kelambakkam ➝ Thiruporur
      </p>
    </div>
  );
};

export default ProjectContent;
